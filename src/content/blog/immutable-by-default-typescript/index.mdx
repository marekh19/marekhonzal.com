...
description: 'Learn how to apply immutability as a default in TypeScript. From as const to value tables, these practical patterns give you safer code, cleaner APIs, and refactors you can trust.'
date: 2025-09-04
isDraft: false
isFeatured: true
tags:
...
I didn't "_go functional_." I just got tired of spooky side-effects and fragile refactors. Immutability wasn't a Big Rewrite - more like **small defaults** that kept paying off. This post is how I use it day-to-day: why it helps, how I think about it, and the exact patterns I paste into real code.
...
This pattern scales beyond static permissions.
Here’s a real-world example from a Todo app: the **filter state** that powers both our business logic and the UI tabs.
...
Once you start [treating strings as _types_](https://www.typescriptlang.org/docs/handbook/2/template-literal-types.html), you can encode intent directly in them.
...
&ldquo;_The new `satisfies` operator lets us validate that the type of an expression matches some type, **without changing the resulting type of that expression**._&rdquo;
...
In practice, shallow immutability is usually enough.
But if you need **deep immutability**, you can define a utility type instead of wrapping each level in `Readonly<T>` manually:
