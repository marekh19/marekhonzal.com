...
description:
  'Learn how to apply immutability as a default in TypeScript. From as const to value
  tables, these practical patterns give you safer code, cleaner APIs, and refactors you
  can trust.'
date: 2025-09-04
isDraft: false
isFeatured: true
...
This pattern scales beyond static permissions. Here’s a real-world example from a Todo
...
- **Single source of truth.** The filter values (`'all' | 'done' | 'not-done'`) live in
  ...
  tabs.
- **Types from values.** `TodoFilter` is derived from `FILTER_STATE`. You can’t pass an
  invalid string anywhere—TypeScript will catch it.
...
- **End-to-end consistency.** The same literal drives your state, your filter functions,
  and your UI. There’s zero chance of your logic and UI drifting apart.

It’s not just about immutability—it’s about **locking values, types, and logic together**.
...
💡 So whenever you use `as const satisfies` with an array, remember to explicitly add
`readonly` if you don’t want to lose it in the type signature.
...
What’s more, `readonly` **removes mutating methods** from the type. You won’t even see
...
That’s the real win: the type system nudges you into writing pure functions and makes
...
When you use `as const`, the entire value becomes deeply immutable—no matter how many
