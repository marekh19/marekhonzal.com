...
**Why this works well**: We treat the `shared-counter` package as a singleton across all three apps. That means they all point to the same store instance. When one app updates the counter, all others see the change instantly. No complex synchronization or messaging layer needed.
